...
try {
    $migration = Join-Path $repoRoot 'database\migrations\001_admin_foundation.sql'
    & $psqlPath -X -v ON_ERROR_STOP=1 -h 127.0.0.1 -p 5432 -U codefy_owner -d codefy_guide -f $migration
    if ($LASTEXITCODE -ne 0) { throw 'Migration failed. The SQL transaction was rolled back; fix the reported issue and rerun this script.' }
