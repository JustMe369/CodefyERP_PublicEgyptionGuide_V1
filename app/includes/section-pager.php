...
<nav class="section-pager flex flex-wrap items-center justify-between gap-4 mt-12 pt-6 border-t border-slate-200">
    <?php if ($nav['prev']): $p = $SECTIONS[$nav['prev']]; ?>
        <a href="<?= e(codefy_section_url($nav['prev'])) ?>" class="inline-flex items-center gap-2 px-4 py-2 bg-slate-100 text-slate-700 font-bold rounded-xl hover:bg-slate-200 transition-colors">
    ...
    <?php if ($nav['next']): $n = $SECTIONS[$nav['next']]; ?>
        <a href="<?= e(codefy_section_url($nav['next'])) ?>" class="inline-flex items-center gap-2 px-5 py-2.5 bg-primary-600 text-white font-bold rounded-xl hover:bg-primary-700 transition-colors shadow-lg shadow-primary-600/20">
